        ...
        return std::visit([&](auto &rhs) -> EvalConstant::Constant {
            ...
            case BinaryOp::mod: {
                if constexpr (!std::is_same_v<int64_t, T1>) {
                    diagEngine.Report(llvm::SMLoc::getFromPointer(binaryExpr->token.ptr), diag::err_constant_expr);
                    return 0;
                }
    ...
    case UnaryOp::bitwise_not:
        return std::visit([&](auto &lhs)->EvalConstant::Constant {
            ...
            if constexpr (!std::is_same_v<int64_t, T>) {
                diagEngine.Report(llvm::SMLoc::getFromPointer(expr->token.ptr), diag::err_constant_expr);
                return 0;
            }else {
                return ~lhs;
            }
...
EvalConstant::Constant EvalConstant::VisitSizeOfExpr(SizeOfExpr *expr) {
    if (expr->type) {
        ...
        return expr->node->type->GetSize() / 8;
    }
}
